...
class AB
{
    ...
    /**
     * Gets a Javascript snippet to register the tests the user is enrolled in
     * @return string Mixpanel JS
     */
    public static function get_mixpanel_enrollment()
    {
        $ret ='<script type="text/javascript">';
        $ret .= 'mixpanel.register({';
        $i = 0;
        foreach (self::get_enrollment() as $test_name=>$variant) {
            $i++;
            $ret .= "'$test_name':'$variant'";
            if ($i !== count(self::get_enrollment())) {
                $ret .= ',';
            }
        }
